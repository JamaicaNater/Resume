...
const EducationController = {
    ...
    createEducation: async (req: Request, res: Response) => {
        try {
            const { name, degreeType, major, minor, gpa, details, startDate, endDate, city, state, country } = req.body;
            const education = new Education({ name, degreeType, major, minor, gpa, details, startDate, endDate, city, state, country, userId: req.session.user?.id });
